<template>
    <NormalPopup :no-border="true" :dimmed="true" width="400px">
        <div slot="header" class="flexed justify-center align-center">
            <strong class="m-t-5 m-b-5">Delete Room</strong>
        </div>
        <div v-loading="loading" class="body">
            <div class="flexed-column">
                <span>Are you sure you want to delete <strong>{{roomProp.type}} - {{roomProp.code}}</strong> room ?</span>
            </div>
        
        </div>
        <div slot="bottom" class="bottom">
            <el-button @click="$emit('close')">Cancel</el-button>
            <el-button type="danger" @click="deleteRoom(roomProp.id)">Delete</el-button>
        </div>
    </NormalPopup>
</template>
