<template>
    <div class="form-max-width" v-if="$route.name === 'addons'" v-loading="loading">
        <div class="shortcuts-header flexed justify-between">
            <h4>Addons</h4>
        </div>
        <div class="one-column-list">
            <div class="flexed justify-between m-b-20">
                <el-input class="search-input" size="big" placeholder="Search addons by name" v-model="query" :style="'max-width:450px'">
                    <i class="el-icon-search el-input__icon" slot="suffix"></i>
                </el-input>
                <div>
                    <el-button size="big" style="background-color:#ff7b50; border-radius:15px;color:white" @click="addEditAddon()">New</el-button>
                </div>
            </div>
       ...
       <el-alert v-if="!loading && filteredAddons && filteredAddons.length === 0"  
            type="info" 
            :closable="false" 
            class="mt-30"
